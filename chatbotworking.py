...
class Chatbot(tk.Frame):
    def __init__(self, root):
        ...
        self.voice_input_button = tk.Button(main_frame, text="Voice Input", command=self.start_voice_recognition)
        self.voice_input_button.pack()
    ...
    # Implement voice recognition logic here
        with sr.Microphone() as source:
            ...
            # Insert the recognized text into the text input field
                ...
                self.text_input.insert(0,tk.END, recognized_text)  # Insert the recognized text

            # Automatically simulate pressing the "Enter" key after inserting the text
                self.text_input.event_generate("<Return>")
    ...
    def listen_for_voice_input(self):
        ...
        try:
        # Recognize the voice input using a recognition engine (e.g., Google Web Speech API)
            recognized_text = recognizer.recognize_google(audio)
            return recognized_text
        except sr.UnknownValueError:
            return "Sorry, I couldn't understand the audio."
        except sr.RequestError as e:
            return f"Sorry, there was an error with the voice recognition service: {e}"
...
# Check if we are collecting additional information about appetite
            ...
            if self.user_age == '':
                ...
                self.text.insert(tk.END, f"\n\nDr. Maharishi Krishna: Are you more active during specific times of the day(more active in the evening,no specific pattern,more active in the morning)?")
        ...
        # Check if we are collecting additional information about activity times
            ...
            # Ask about sleep hours
                self.text.insert(tk.END, "\n\nDr. Maharishi Krishna:  On average, how many hours of sleep do you typically get each night(Less than 6 hours,6-8 hours,More than 8 hours)?")
        # Check if we are collecting additional information about sleep patterns
            elif self.user_sleep_hours == '':
...
class DoshaCalculator:
    ...
    def get_user_attributes(self):
        ...
        user_attributes_list = calculator.get_user_attributes()

# Print the list of user attribute values
        print(user_attributes_list)
